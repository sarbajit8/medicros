const PaymentRemarks = require("../../models/PaymentRemarks");

// Add or Update Payment Remarks
const upsertPaymentRemarks = async (req, res) => {
  try {
    ...
    let existing = await PaymentRemarks.findOne({ employeeId, date: logDate });
    ...
    if (existing) {
      ...
      return res.status(200).json({
        success: true,
        message: "Payment remarks updated successfully.",
        data: existing,
      });
    } else {
      // Create new record
      const newRemarks = new PaymentRemarks({
        employeeId,
        date: logDate,
        accounremarks,
        accountamount,
      ...
      return res.status(201).json({
        success: true,
        message: "Payment remarks created successfully.",
        data: newRemarks,
      });
    }
  } catch (error) {
...
// Get Payment Remarks by employeeId and date
const getPaymentRemarksByEmployeeAndDate = async (req, res) => {
  try {
    ...
    const data = await PaymentRemarks.findOne({ employeeId, date: logDate });

    if (!data) {
      return res.status(404).json({
        success: false,
        message: "No payment remarks found for the given employee and date.",
      });
    }

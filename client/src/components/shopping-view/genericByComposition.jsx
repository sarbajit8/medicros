...
const MedicineCard = ({ medicine }) => {
  ...
  return (
    <div className="bg-white shadow-md hover:shadow-lg rounded-xl border border-gray-200 w-full md:w-80 transition-transform transform hover:scale-105">
      {/* Header */}
      <div className="bg-gradient-to-r from-blue-600 to-teal-500 text-white p-3 rounded-t-xl font-semibold text-lg uppercase">
        🏥 {medicine?.title}
      </div>
      ...
      <div className="p-4 space-y-3">
        ...
        <div className="flex items-center space-x-3 mt-3">
          ...
          {/* <button className="px-5 py-2 text-white text-sm font-semibold rounded-lg bg-green-600 hover:bg-green-700 transition"
...
const GenericByComposition = () => {
  const { productList } = useSelector((state) => state.adminProducts);
  const dispatch = useDispatch();
  const { composition } = useParams();
  ...
  const filteredProducts = productList.filter(
    (medicine) => normalizeText(medicine?.composition) === normalizeText(composition)
  );
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <h1 className="text-2xl font-extrabold text-gray-800 uppercase">{composition}</h1>
...
export default GenericByComposition;

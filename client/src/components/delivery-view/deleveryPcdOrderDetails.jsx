...
function DeleveryPcdOrderDetailsView({ orderDetails }) {
  ...
  const handleUpdateStatus = (event) => {
    ...
    dispatch(updatePcdOrderStatus({ id: orderDetails?._id, orderStatus: formData.status }))
      .then((data) => {
        if (data?.payload?.success) {
          dispatch(getPcdOrderDetailsForAdmin(orderDetails?._id));
  ...
  const handleUpdatePaymentStatus = () => {
    ...
    dispatch(updatePcdPaymentStatus({ orderId: orderDetails?._id, paymentStatus }))
      .then((data) => {
        dispatch(getPcdOrderDetailsForAdmin(orderDetails?._id));
        if (data?.payload?.success) {
  ...
  const handleUpdatePaymentMethod = () => {
    ...
    dispatch(updatePcdPaymentMethod({ orderId: orderDetails?._id, paymentMethod }))
      .then((data) => {
        dispatch(getPcdOrderDetailsForAdmin(orderDetails?._id));
        if (data?.payload?.success) {
  ...
  dispatch(updatePcdOrderDue({ orderId: orderDetails?._id, due }))
    .then((data) => {
      if (data?.payload?.success) {
...
export default DeleveryPcdOrderDetailsView;
